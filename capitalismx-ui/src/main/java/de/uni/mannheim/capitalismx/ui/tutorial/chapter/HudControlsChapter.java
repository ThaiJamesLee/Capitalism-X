...
public class HudControlsChapter extends TutorialChapter {
	...
	public HudControlsChapter() {
		...
		pages.add(new TutorialPageBuilder(this).setTargetNode(nodes.get(2)).setMessageKey("chapter.controls.hud.pause").setEndTrigger(NextPageTrigger.CLICK).build());
		pages.add(new TutorialPageBuilder(this).setTargetNode(nodes.get(3)).setMessageKey("chapter.controls.hud.networth").setArrowLocation(ArrowLocation.BOTTOM_CENTER).setEndTrigger(NextPageTrigger.CONFIRM).build());
